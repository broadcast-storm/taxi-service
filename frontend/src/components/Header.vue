<template>
    <!-- mobile menu bar -->
    <div class="bg-gray-900 text-gray-100 flex justify-between md:hidden z-50">
        <!-- logo -->
        <div class="flex items-center pl-2">
            <Logo class="w-8" />
            <span class="pl-2 text-yellow-500 font-bold">Сервис Такси</span>
        </div>

        <!-- mobile menu button -->
        <button
            class="
                mobile-menu-button
                p-4
                focus:outline-none focus:bg-yellow-500
            "
            @click="toggleNavBar()"
        >
            <svg
                class="h-5 w-5"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke="white"
            >
                <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M4 6h16M4 12h16M4 18h16"
                />
            </svg>
        </button>
    </div>

    <!-- sidebar -->
    <div
        class="
            sidebar
            flex flex-col
            justify-between
            bg-yellow-500
            text-gray-900
            w-64
            py-7
            px-2
            absolute
            inset-y-0
            left-0
            transform
            md:relative md:translate-x-0
            transition
            duration-200
            ease-in-out
            z-50
        "
        :class="{ '-translate-x-full': isOpen }"
    >
        <!-- logo -->
        <div class="space-y-8">
            <router-link
                :to="routesList.mainPage.path"
                class="text-gray-900 flex items-center space-x-2 px-4"
            >
                <Logo class="w-8" is-black />
                <span class="text-2xl font-bold">Сервис Такси</span>
            </router-link>
            <div class="bg-gray-900 rounded py-4 px-4">
                <template v-if="true">
                    <div class="text-yellow-500 flex items-center mb-4">
                        <svg
                            class="h-8 fill-current mr-2"
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 350 350"
                        >
                            <g>
                                <path
                                    d="M175,171.173c38.914,0,70.463-38.318,70.463-85.586C245.463,38.318,235.105,0,175,0s-70.465,38.318-70.465,85.587
		C104.535,132.855,136.084,171.173,175,171.173z"
                                />
                                <path
                                    d="M41.909,301.853C41.897,298.971,41.885,301.041,41.909,301.853L41.909,301.853z"
                                />
                                <path
                                    d="M308.085,304.104C308.123,303.315,308.098,298.63,308.085,304.104L308.085,304.104z"
                                />
                                <path
                                    d="M307.935,298.397c-1.305-82.342-12.059-105.805-94.352-120.657c0,0-11.584,14.761-38.584,14.761
		s-38.586-14.761-38.586-14.761c-81.395,14.69-92.803,37.805-94.303,117.982c-0.123,6.547-0.18,6.891-0.202,6.131
		c0.005,1.424,0.011,4.058,0.011,8.651c0,0,19.592,39.496,133.08,39.496c113.486,0,133.08-39.496,133.08-39.496
		c0-2.951,0.002-5.003,0.005-6.399C308.062,304.575,308.018,303.664,307.935,298.397z"
                                />
                            </g>
                        </svg>
                        <span class="text-lg">Вася Пукин</span>
                        <router-link
                            :to="routesList.mainPage.children.profilePage"
                            class="ml-auto text-gray-300"
                            ><svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512"
                                class="h-4 fill-current"
                            >
                                <g>
                                    <g>
                                        <path
                                            d="M496.659,312.107l-47.061-36.8c0.597-5.675,1.109-12.309,1.109-19.328c0-7.019-0.491-13.653-1.109-19.328l47.104-36.821
			c8.747-6.912,11.136-19.179,5.568-29.397L453.331,85.76c-5.227-9.557-16.683-14.464-28.309-10.176l-55.531,22.293
			c-10.645-7.68-21.803-14.165-33.344-19.349l-8.448-58.901C326.312,8.448,316.584,0,305.086,0h-98.133
			c-11.499,0-21.205,8.448-22.571,19.456l-8.469,59.115c-11.179,5.035-22.165,11.435-33.28,19.349l-55.68-22.357
			C76.52,71.531,64.04,76.053,58.856,85.568L9.854,170.347c-5.781,9.771-3.392,22.464,5.547,29.547l47.061,36.8
			c-0.747,7.189-1.109,13.44-1.109,19.307s0.363,12.117,1.109,19.328l-47.104,36.821c-8.747,6.933-11.115,19.2-5.547,29.397
			l48.939,84.672c5.227,9.536,16.576,14.485,28.309,10.176l55.531-22.293c10.624,7.659,21.781,14.144,33.323,19.349l8.448,58.88
			C185.747,503.552,195.454,512,206.974,512h98.133c11.499,0,21.227-8.448,22.592-19.456l8.469-59.093
			c11.179-5.056,22.144-11.435,33.28-19.371l55.68,22.357c2.688,1.045,5.483,1.579,8.363,1.579c8.277,0,15.893-4.523,19.733-11.563
			l49.152-85.12C507.838,331.349,505.448,319.083,496.659,312.107z M256.019,341.333c-47.061,0-85.333-38.272-85.333-85.333
			s38.272-85.333,85.333-85.333s85.333,38.272,85.333,85.333S303.08,341.333,256.019,341.333z"
                                        />
                                    </g>
                                </g>
                            </svg>
                        </router-link>
                    </div>
                    <router-link
                        :to="routesList.mainPage.children.profilePage"
                        class="
                            group
                            relative
                            w-full
                            flex
                            justify-center
                            py-2
                            border-2 border-yellow-500
                            text-sm
                            font-medium
                            rounded
                            text-yellow-500
                            bg-transparent
                            hover:bg-yellow-500 hover:text-white
                            focus:outline-none
                            focus:ring-2
                            focus:ring-offset-2
                            focus:ring-white
                        "
                        >Заказать машину
                    </router-link>
                </template>
                <template v-else>
                    <router-link
                        :to="routesList.authPage.children.signUpPage.path"
                        class="
                            group
                            relative
                            w-full
                            flex
                            justify-center
                            py-2
                            border-2 border-yellow-500
                            text-sm
                            font-medium
                            rounded
                            text-yellow-500
                            bg-transparent
                            hover:bg-yellow-500 hover:text-white
                            focus:outline-none
                            focus:ring-2
                            focus:ring-offset-2
                            focus:ring-white
                        "
                    >
                        Зарегистрироваться
                    </router-link>
                    <div class="text-sm text-white text-center mt-2">
                        Уже есть аккаунт?
                        <router-link
                            :to="routesList.authPage.children.signInPage.path"
                            class="font-medium text-yellow-400 hover:text-white"
                        >
                            Войти!
                        </router-link>
                    </div>
                </template>
            </div>
            <!-- nav -->
            <nav class="space-y-4">
                <router-link
                    :to="routesList.mainPage.path"
                    exact-active-class="text-white"
                    class="
                        flex
                        items-center
                        py-2.5
                        px-4
                        rounded
                        transition
                        duration-200
                        hover:bg-gray-900 hover:text-white
                    "
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 460.298 460.297"
                        class="h-6 fill-current mr-2"
                    >
                        <g>
                            <g>
                                <path
                                    d="M230.149,120.939L65.986,256.274c0,0.191-0.048,0.472-0.144,0.855c-0.094,0.38-0.144,0.656-0.144,0.852v137.041
			c0,4.948,1.809,9.236,5.426,12.847c3.616,3.613,7.898,5.431,12.847,5.431h109.63V303.664h73.097v109.64h109.629
			c4.948,0,9.236-1.814,12.847-5.435c3.617-3.607,5.432-7.898,5.432-12.847V257.981c0-0.76-0.104-1.334-0.288-1.707L230.149,120.939
			z"
                                />
                                <path
                                    d="M457.122,225.438L394.6,173.476V56.989c0-2.663-0.856-4.853-2.574-6.567c-1.704-1.712-3.894-2.568-6.563-2.568h-54.816
			c-2.666,0-4.855,0.856-6.57,2.568c-1.711,1.714-2.566,3.905-2.566,6.567v55.673l-69.662-58.245
			c-6.084-4.949-13.318-7.423-21.694-7.423c-8.375,0-15.608,2.474-21.698,7.423L3.172,225.438c-1.903,1.52-2.946,3.566-3.14,6.136
			c-0.193,2.568,0.472,4.811,1.997,6.713l17.701,21.128c1.525,1.712,3.521,2.759,5.996,3.142c2.285,0.192,4.57-0.476,6.855-1.998
			L230.149,95.817l197.57,164.741c1.526,1.328,3.521,1.991,5.996,1.991h0.858c2.471-0.376,4.463-1.43,5.996-3.138l17.703-21.125
			c1.522-1.906,2.189-4.145,1.991-6.716C460.068,229.007,459.021,226.961,457.122,225.438z"
                                />
                            </g>
                        </g>
                    </svg>
                    <span class="font-medium">Главная</span>
                </router-link>

                <router-link
                    :to="routesList.mainPage.children.newsPage"
                    active-class="text-white"
                    class="
                        flex
                        items-center
                        py-2.5
                        px-4
                        rounded
                        transition
                        duration-200
                        hover:bg-gray-900 hover:text-white
                    "
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 512 512"
                        class="h-6 fill-current mr-2"
                    >
                        <g>
                            <g>
                                <path
                                    d="M496.807,10.888H15.193C6.802,10.888,0,17.691,0,26.081v459.838c0,8.391,6.802,15.193,15.193,15.193h481.614
			c8.391,0,15.193-6.802,15.193-15.193V26.081C512,17.691,505.198,10.888,496.807,10.888z M481.614,470.726H30.386V41.274h451.229
			V470.726z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M126.793,82.146v29.372l-14.141-26.971c-5.029-9.66-8.438-11.205-16.804-11.205c-7.541,0-12.607,3.588-12.607,8.927
			v77.903c0.001,11.915,25.216,11.926,25.216,0v-29.865l17.604,32.058c3.243,5.969,8.832,6.734,13.339,6.734
			c7.423,0,12.608-3.672,12.608-8.927V82.146C152.009,70.342,126.793,70.338,126.793,82.146z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M222.33,146.337h-29.84v-14.779h15.363c11.808,0,11.655-20.799,0-20.799H192.49V96.103h29.84
			c11.653,0,11.868-22.762,0-22.762h-43.674c-6.595,0-11.38,3.755-11.38,8.927v77.903c-0.001,5.173,4.786,8.927,11.38,8.927h43.674
			C234.202,169.099,233.972,146.337,222.33,146.337z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M328.964,79.073l-15.387,51.931l-7.072-30.258c-2.199-9.896-22.148-9.845-24.328-0.032l-7.08,30.291L259.718,79.1
			c-3.288-11.502-27.919-4.096-24.091,7.421l24.404,74.807c3.656,11.628,28.287,11.153,30.895-0.26l3.41-15.548l3.417,15.574
			c2.556,11.187,26.836,12.055,30.864,0.308l24.422-74.86C356.896,74.982,332.233,67.629,328.964,79.073z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M385.581,99.199c0-2.625,3.623-4.323,9.229-4.323c9.425,0,11.14,4.662,16.807,4.662c7.523,0,10.645-8.531,10.645-13.099
			c0-12.837-21.012-13.835-27.453-13.835c-15.89,0-34.445,7.447-34.445,28.434c0,30.115,38.89,29.69,38.89,40.976
			c0,4.916-7.955,5.058-8.862,5.058c-10.413,0-13.396-9.078-20.856-9.078c-6.842,0-11.38,8.032-11.38,13.343
			c0,22.501,66.314,31.862,66.314-11.286C424.471,108.085,385.581,107.524,385.581,99.199z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M204.019,277.633H79.655c-8.391,0-15.193,6.802-15.193,15.193v140.499c0,8.391,6.801,15.193,15.193,15.193h124.364
			c8.391,0,15.193-6.802,15.193-15.193V292.826C219.212,284.435,212.41,277.633,204.019,277.633z M188.826,418.131H94.848V308.018
			h93.978V418.131z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M422.575,202.515H79.655c-8.391,0-15.193,6.802-15.193,15.193s6.801,15.193,15.193,15.193h342.919
			c8.391,0,15.193-6.802,15.193-15.193S430.965,202.515,422.575,202.515z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M422.278,277.433H291.577c-8.391,0-15.193,6.802-15.193,15.193s6.802,15.193,15.193,15.193h130.701
			c8.391,0,15.193-6.802,15.193-15.193S430.668,277.433,422.278,277.433z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M422.278,342.913H291.577c-8.391,0-15.193,6.802-15.193,15.193s6.802,15.193,15.193,15.193h130.701
			c8.391,0,15.193-6.802,15.193-15.193S430.668,342.913,422.278,342.913z"
                                />
                            </g>
                        </g>
                        <g>
                            <g>
                                <path
                                    d="M422.278,408.393H291.577c-8.391,0-15.193,6.802-15.193,15.193s6.802,15.193,15.193,15.193h130.701
			c8.391,0,15.193-6.802,15.193-15.193S430.668,408.393,422.278,408.393z"
                                />
                            </g>
                        </g>
                    </svg>

                    <span class="font-medium">Новости</span>
                </router-link>
                <router-link
                    :to="routesList.mainPage.children.pricesPage"
                    active-class="text-white"
                    class="
                        flex
                        items-center
                        py-2.5
                        px-4
                        rounded
                        transition
                        duration-200
                        hover:bg-gray-900 hover:text-white
                    "
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-6 fill-current mr-2"
                        viewBox="-25 0 512 512.00078"
                    >
                        <g>
                            <path
                                d="M 266.4375 104.960938 L 322.957031 104.960938 L 236.449219 13.957031 L 236.449219 74.855469 C 236.449219 91.351562 249.828125 104.960938 266.4375 104.960938 Z M 266.4375 104.960938 "
                            />
                            <path
                                d="M 172.667969 334.488281 L 72.433594 334.488281 C 67.359375 334.488281 63.207031 330.339844 63.207031 325.261719 C 63.207031 320.1875 67.359375 316.035156 72.433594 316.035156 L 178.78125 316.035156 C 184.660156 301.042969 192.503906 288.351562 202.195312 275.664062 L 72.433594 275.664062 C 67.359375 275.664062 63.207031 271.515625 63.207031 266.4375 C 63.207031 261.363281 67.359375 257.210938 72.433594 257.210938 L 219.726562 257.210938 C 247.40625 232.988281 284.085938 217.535156 324.109375 217.535156 C 327.914062 217.535156 331.03125 217.878906 335.644531 218.226562 L 335.644531 123.414062 L 266.4375 123.414062 C 239.679688 123.414062 217.996094 101.5 217.996094 74.855469 L 217.996094 0 L 41.0625 0 C 18.222656 0 0 18.683594 0 41.636719 L 0 393.660156 C 0 416.613281 18.222656 434.835938 41.0625 434.835938 L 180.046875 434.835938 C 172.089844 416.382812 167.707031 395.621094 167.707031 373.9375 C 167.589844 360.328125 169.4375 347.175781 172.667969 334.488281 Z M 72.433594 199.539062 L 189.277344 199.539062 C 194.351562 199.539062 198.503906 203.691406 198.503906 208.769531 C 198.503906 213.84375 194.351562 217.996094 189.277344 217.996094 L 72.433594 217.996094 C 67.359375 217.996094 63.207031 213.84375 63.207031 208.769531 C 63.207031 203.691406 67.359375 199.539062 72.433594 199.539062 Z M 72.433594 199.539062 "
                            />
                            <path
                                d="M 323.992188 236.335938 C 247.984375 236.335938 186.160156 298.15625 186.160156 374.167969 C 186.160156 450.175781 247.984375 512 323.992188 512 C 400.003906 512 461.828125 450.175781 461.828125 374.167969 C 461.828125 298.15625 400.003906 236.335938 323.992188 236.335938 Z M 323.765625 364.25 C 351.5625 364.25 374.050781 386.855469 374.050781 414.535156 C 374.050781 439.449219 355.945312 460.097656 332.183594 464.132812 L 332.183594 477.511719 C 332.183594 482.589844 328.03125 486.742188 322.957031 486.742188 C 317.882812 486.742188 313.730469 482.589844 313.730469 477.511719 L 313.730469 463.789062 C 290.777344 459.058594 273.476562 438.757812 273.476562 414.535156 C 273.476562 409.460938 277.628906 405.308594 282.703125 405.308594 C 287.777344 405.308594 291.929688 409.460938 291.929688 414.535156 C 291.929688 432.070312 306.234375 446.371094 323.765625 446.371094 C 341.296875 446.371094 355.597656 432.066406 355.597656 414.535156 C 355.597656 397.003906 341.296875 382.703125 323.765625 382.703125 C 295.96875 382.703125 273.476562 360.09375 273.476562 332.414062 C 273.476562 308.074219 290.777344 287.777344 313.730469 283.164062 L 313.730469 273.359375 C 313.730469 268.28125 317.882812 264.128906 322.957031 264.128906 C 328.03125 264.128906 332.183594 268.28125 332.183594 273.359375 L 332.183594 282.699219 C 355.945312 286.738281 374.050781 307.382812 374.050781 332.296875 C 374.050781 337.371094 369.902344 341.523438 364.824219 341.523438 C 359.75 341.523438 355.597656 337.371094 355.597656 332.296875 C 355.597656 315.457031 342.449219 301.617188 325.839844 300.464844 C 324.917969 300.808594 323.878906 300.925781 322.839844 300.925781 C 321.917969 300.925781 321.109375 300.808594 320.1875 300.578125 C 304.269531 302.308594 291.816406 315.804688 291.816406 332.296875 C 291.929688 349.945312 306.230469 364.25 323.765625 364.25 Z M 323.765625 364.25 "
                            />
                        </g>
                    </svg>
                    <span class="font-medium">Цены</span>
                </router-link>
                <router-link
                    :to="routesList.mainPage.children.driversPage"
                    active-class="text-white"
                    class="
                        flex
                        items-center
                        py-2.5
                        px-4
                        rounded
                        transition
                        duration-200
                        hover:bg-gray-900 hover:text-white
                    "
                    ><svg
                        viewBox="0 0 512 512"
                        class="h-6 fill-current mr-2"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <g>
                            <g>
                                <path
                                    d="m96.559 150h318.882c13.326-12.75 20.559-27.244 20.559-42.905 0-55.049-87.48-107.095-180-107.095s-180 52.046-180 107.095c0 15.661 7.233 30.155 20.559 42.905zm101.961-83.32c4.556-6.885 13.857-8.745 20.801-4.16l36.679 24.448 36.68-24.448c6.899-4.585 16.201-2.725 20.801 4.16 4.6 6.899 2.739 16.201-4.16 20.801l-45 30c-2.52 1.685-5.42 2.52-8.32 2.52s-5.801-.835-8.32-2.52l-45-30c-6.901-4.6-8.761-13.902-4.161-20.801z"
                                />
                                <path
                                    d="m138.754 180c10.891 34.581 58.262 60 117.246 60s106.355-25.419 117.246-60z"
                                />
                            </g>
                            <g>
                                <path
                                    d="m201.806 512h108.387l28.085-84.26c-13.573 32.362-45.884 54.26-82.278 54.26-36.357 0-68.681-21.982-82.546-55.052z"
                                />
                                <path
                                    d="m130.194 392h-9.194c-57.891 0-105 47.109-105 105 0 8.291 6.709 15 15 15h139.195z"
                                />
                                <path
                                    d="m391 392h-9.197l-39.998 120h139.195c8.291 0 15-6.709 15-15 0-57.708-46.84-105-105-105z"
                                />
                            </g>
                            <path
                                d="m256 270c-49.759 0-92.873-15.645-120-40.915v42.915c0 42.642 23.262 82.295 60 103.55v16.45c0 32.245 26.105 60 60 60 33.847 0 60-27.589 60-60v-16.45c36.738-21.255 60-60.908 60-103.55v-42.915c-27.127 25.27-70.241 40.915-120 40.915zm56.572 51.995c-8.452 23.936-31.186 40.005-56.572 40.005s-48.12-16.069-56.572-40.005c-2.769-7.808 1.318-16.377 9.141-19.131 7.793-2.754 16.377 1.333 19.131 9.141 4.233 11.953 15.614 19.995 28.3 19.995s24.067-8.042 28.301-19.995c2.739-7.822 11.382-11.909 19.131-9.141 7.822 2.754 11.909 11.324 9.14 19.131z"
                            />
                        </g>
                    </svg>
                    <span class="font-medium">Водители</span>
                </router-link>
            </nav>
        </div>
        <div
            class="
                flex
                items-center
                py-2.5
                px-4
                rounded
                transition
                duration-200
                hover:bg-gray-900 hover:text-white
                cursor-pointer
            "
        >
            <svg
                viewBox="0 0 512.00533 512"
                xmlns="http://www.w3.org/2000/svg"
                class="h-6 fill-current mr-2 mt-auto"
            >
                <path
                    d="m320 277.335938c-11.796875 0-21.332031 9.558593-21.332031 21.332031v85.335937c0 11.753906-9.558594 21.332032-21.335938 21.332032h-64v-320c0-18.21875-11.605469-34.496094-29.054687-40.554688l-6.316406-2.113281h99.371093c11.777344 0 21.335938 9.578125 21.335938 21.335937v64c0 11.773438 9.535156 21.332032 21.332031 21.332032s21.332031-9.558594 21.332031-21.332032v-64c0-35.285156-28.714843-63.99999975-64-63.99999975h-229.332031c-.8125 0-1.492188.36328175-2.28125.46874975-1.027344-.085937-2.007812-.46874975-3.050781-.46874975-23.53125 0-42.667969 19.13281275-42.667969 42.66406275v384c0 18.21875 11.605469 34.496093 29.054688 40.554687l128.386718 42.796875c4.351563 1.34375 8.679688 1.984375 13.226563 1.984375 23.53125 0 42.664062-19.136718 42.664062-42.667968v-21.332032h64c35.285157 0 64-28.714844 64-64v-85.335937c0-11.773438-9.535156-21.332031-21.332031-21.332031zm0 0"
                />
                <path
                    d="m505.75 198.253906-85.335938-85.332031c-6.097656-6.101563-15.273437-7.9375-23.25-4.632813-7.957031 3.308594-13.164062 11.09375-13.164062 19.714844v64h-85.332031c-11.777344 0-21.335938 9.554688-21.335938 21.332032 0 11.777343 9.558594 21.332031 21.335938 21.332031h85.332031v64c0 8.621093 5.207031 16.40625 13.164062 19.714843 7.976563 3.304688 17.152344 1.46875 23.25-4.628906l85.335938-85.335937c8.339844-8.339844 8.339844-21.824219 0-30.164063zm0 0"
                />
            </svg>
            <span class="font-medium">Выйти</span>
        </div>
    </div>
</template>

<script>
import routesList from '@/router/routesList'
import Logo from '@/components/Logo'

export default {
    name: 'Header',
    components: { Logo },
    data() {
        return {
            routesList,
            isOpen: true,
        }
    },
    methods: {
        toggleNavBar() {
            this.isOpen = !this.isOpen
        },
    },
}
</script>

<style lang="scss" scoped></style>
